...
export class StockReorderService {
  ...
  getObjectList(dateFrom: string, dateTo: string) {
    return this.http.get<StockReorderList[]>(this.configService.selected_sys_param.apiUrl + `MobileStockReorder/srlist/${dateFrom}/${dateTo}`);
  }

  getObjectListByDate(trxDate: string, locationCode: string) {
    return this.http.get<StockReorderList[]>(this.configService.selected_sys_param.apiUrl + `MobileStockReorder/srlist/${trxDate}/${locationCode}`);
  }
