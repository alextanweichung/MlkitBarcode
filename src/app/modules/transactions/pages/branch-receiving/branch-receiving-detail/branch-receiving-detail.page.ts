...
import { BranchReceivingLine, BranchReceivingRoot } from '../../../models/branch-receiving';
...
export class BranchReceivingDetailPage implements OnInit {
  ...
  loadDetail() {
    try {
      this.objectService.getBranchReceivingDetail(this.objectId).subscribe(response => {
        this.object = response;
        this.flattenBranchReceiving = this.objectService.unflattenDtoDetail(this.object);
      }, error => {
        console.error(error);
      })
    } catch (error) {
  ...
  getFlattenVariations(itemId: number): BranchReceivingLine[] {
    return this.flattenBranchReceiving.details.filter(r => r.itemId === itemId);
  }
  ...
  async downloadPdf() {
    try {
      ...
      this.objectService.downloadPdf("IMPC002", "pdf", this.object.header.branchReceivingId).subscribe(async response => {
