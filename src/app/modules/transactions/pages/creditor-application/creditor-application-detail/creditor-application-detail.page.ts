...
import { CreditorApplicationRoot } from '../../../models/creditor-application';
...
@Component({
  selector: 'app-creditor-application-detail',
  templateUrl: './creditor-application-detail.page.html',
  styleUrls: ['./creditor-application-detail.page.scss'],
})
export class CreditorApplicationDetailPage implements OnInit {

  objectId: number
  object: CreditorApplicationRoot;
  ...
  loadObject() {
    try {
      this.objectService.getObjectById(this.objectId).subscribe(response => {
        ...
        this.loadWorkflow(this.object.header.vendorPreId);
      }, error => {
        console.error(error);
      })
    } catch (e) {
  ...
  showStatus() {
    if (this.workflowState.length === 0) {
      ...
      if (currenctStatus && currenctStatus.title) {
        ...
        this.toastService.presentToast("Doc Status", "Workflow completed, creditor generated.", "top", "success", 1000);
      }
    }
  }
  ...
  async startUpload() {
    if (this.file) {
      try {
        ...
        this.objectService.uploadFile(this.object.header.vendorPreId, 0, formData).subscribe(response => {
  ...
  async downloadPdf(reportName: string) {
    try {
      this.objectService.downloadPdf("FAAR006", "pdf", this.object.header.vendorPreId, reportName).subscribe(response => {
  ...
  editObject() {
    let navigationExtras: NavigationExtras = {
      queryParams: {
        objectId: this.object.header.vendorPreId
      }
    }
    this.navController.navigateRoot("/transactions/creditor-application/creditor-application-edit", navigationExtras);
  }

  /* #endregion */

  toggleObject() {
    this.objectService.toggleObject(this.object.header.vendorPreId).subscribe(response => {
