   ...
   loadObjects() {
      try {
         this.objectService.getObjectListByDate(format(this.startDate, "yyyy-MM-dd"), format(this.endDate, "yyyy-MM-dd")).subscribe(async response => {
            console.log("🚀 ~ file: packing.page.ts:71 ~ PackingPage ~ this.objectService.getObjectListByDate ~ response:", response)
            this.objects = response;
            let dates = [...new Set(this.objects.map(obj => this.commonService.convertDateFormatIgnoreTime(new Date(obj.trxDate))))];
            this.uniqueGrouping = dates.map(r => r.getTime()).filter((s, i, a) => a.indexOf(s) === i).map(s => new Date(s));
            await this.uniqueGrouping.sort((a, c) => { return a < c ? 1 : -1 });
            console.log("🚀 ~ file: packing.page.ts:68 ~ PackingPage ~ this.objectService.getObjectListByDate ~ this.uniqueGrouping:", this.uniqueGrouping)
