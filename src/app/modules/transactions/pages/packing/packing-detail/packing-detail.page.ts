...
export class PackingDetailPage implements OnInit, ViewWillEnter {
   ...
   getItemOfSO(salesOrderNum: string) {
      let found = this.object.outstandingPackList.filter(r => r.salesOrderNum === salesOrderNum);
   ...
   editObject() {
      this.objectService.setPackingSummary(this.object);
   ...
   showDetails(rowData: MultiPackingCarton) {
      this.object.details.filter(r => r.cartonBarcode !== rowData.cartonBarcode).flatMap(r => r.isSelected = false);
