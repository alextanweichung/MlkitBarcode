...
export class SalesOrderItemPage implements OnInit, ViewDidEnter {
  ...
  ngOnInit() {
    ...
    console.log("🚀 ~ file: item.page.ts ~ line 42 ~ SalesOrderItemPage ~ ngOnInit ~ this.salesOrderHeader", this.salesOrderHeader)
    this.itemInCart = this.salesOrderService.itemInCart;
  ...
  async searchItem() {
    if (this.itemSearchText && this.itemSearchText.length > 2) {
      ...
      this.salesOrderService.getItemListWithTax(this.itemSearchText, this.datePipe.transform(this.salesOrderHeader.trxDate, 'yyyy-MM-dd'), this.salesOrderHeader.customerId, this.salesOrderHeader.locationId).subscribe(async response => {
        ...
        console.log("🚀 ~ file: item.page.ts ~ line 96 ~ SalesOrderItemPage ~ this.salesOrderService.getItemListWithTax ~ this.availableItem", this.availableItem)
