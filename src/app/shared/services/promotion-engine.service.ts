...
export class PromotionEngineService {
   ...
   for(let event of promotionObject){        
     ...
     if((affectedSalesBillLine.length > 0 && eligibleAmtCheck ) || (eventItemType == "A" && itemRuleTypeCheck == "Q" && eligibleAmtCheck)){     
       ...
       if(event.line.length>0){
         ...
         salesBillLine.forEach(line =>{
           line = this.addInitialDiscountForReplacePromoImpactedLine(line, useTax, isItemPriceTaxInclusive, isDisplayTaxInclusive, roundingPrecision);
         })
       }else{
 ...
 addInitialDiscountForReplacePromoImpactedLine(salesBillLine: TransactionDetail, useTax: boolean, isItemPriceTaxInclusive:boolean, isDisplayTaxInclusive: boolean, roundingPrecision: number){
