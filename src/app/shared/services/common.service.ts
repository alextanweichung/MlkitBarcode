...
export class CommonService {
      ...
      try {
         ...
         if (useTax) {
            if (isItemPriceTaxInclusive) {
               ...
               trxLine.subTotalExTax = (quantity.mul(this.computeAmtExclTax(unitPrice, trxLine.taxPct))).sub(trxLine.discountAmtExTax).toDecimalPlaces(roundingPrecision).toNumber();
         ...
         console.log("🚀 ~ file: common.service.ts:440 ~ CommonService ~ computeDiscTaxAmount ~ trxLine:", trxLine)
         return trxLine;
      } catch (e) {
         console.error(e);
      }
   ...
   computeMarginAmtByConsignmentConfig(trxLine: TransactionDetail, objectHeader: any, computeInvoiceAmt: boolean): TransactionDetail {
