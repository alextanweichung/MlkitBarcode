   ...
   intercept(request: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
      ...
      setTimeout(async () => {
         if (!finished) {
            ...
            //    message: '<p>Loading...</p><span>Please be patient.</span>',
            //    spinner: 'crescent'
            // });
      ...
      return next.handle(authReq).pipe(
         catchError(error => {
            console.log("🚀 ~ file: error-handler.interceptor.ts ~ line 110 ~ ErrorHandlerInterceptor ~ intercept ~ error", error)
            if (error) {
               switch (error.status) {
                  case 400:
                     if (error.error) {
                        ...
                        this.toastService.presentToast('Error', 'Bad request error', 'top', 'danger', 2000);
                        // this.messageService.add({ severity: 'error', summary: 'Bad request error', detail: error.status + ':' + error.statusText });
                     }
                     break;
                  case 401:
                     if (error.error.description) {
                        this.toastService.presentToast('Error', 'Unauthorised', 'top', 'danger', 2000);
                  ...
                  case 404:
                     ...
                     this.toastService.presentToast('Error', 'No result', 'top', 'danger', 2000);
                  ...
                  case 500:
                     ...
                     this.toastService.presentToast('Error', 'Internal server error 500', 'top', 'danger', 2000);
